...
/* 					  //swal(`Nom client : ${value}`);
	...
	<body bgcolor='azure' style="" > 
		...
		//<a href='' onclick='edition1();return false;' id='lien1' style='background-color: gold;color:#444739;font-style:italic;text-decoration:none;border-radius: 5px;-moz-border-radius: 5px;-webkit-border-radius: 5px;'>Ajouter une table </a>&nbsp;";
			...
			<span style='color:#ff103c;font-weight:bold;font-size:75%;'>Serveurs</span></a> &nbsp;|<a href='servir.php?menuParent=".$_SESSION['menuParenT']."&tab=1&trie=1' style='text-decoration:none;font-size:1.1em;'>
			<span style='color:#ff103c;font-weight:bold;font-size:75%;'>Trier</span></a>";
		echo "</span>";	
		...
		if(!empty($_GET['trie'])){
			...
			while($data=mysqli_fetch_array($reqsel))
				...
				if(($j==1)||(($j!=0)&&($array[$j-1]!=$i))){				
					...
					<tr>
						<td style='padding: 10px;padding-bottom: 10px;'><a href='' class='info2' id='test'>"; if(count($checkFormaHour)==3) echo "<span style='color:white;display:block;float:top;margin-left:20px;'>".$Heure."</span>"; //else echo "<span style='color:blue;display:block;float:top;margin-bottom:-25px;'>&nbsp;</span>";
		...
		{ if($i<10) $ii="0".$i; else $ii=$i;
			...
			$Serveurassoc=!empty($data1['Serveurassoc'])?$data1['Serveurassoc']:$data['Serveurassoc'];
			echo "
			<table  WIDTH='' style='border-spacing: 5px 5px;border:3px solid white;"; if($i%5!=0) echo "float:left;";  echo "' class=''>
				<tr>
					<td style='padding: 10px;padding-bottom: 10px;'><a href='' class='info2' id='test'>";  if(!empty($serv)) echo "<span style='color:blue;display:block;float:top;margin-left:20px;'>".$Serveurassoc."</span>"; //else echo "<span style='color:blue;display:block;float:top;margin-bottom:-25px;'>&nbsp;</span>";
						echo "<input type='submit' class='bouton5' id='full' name='table' value='".$ii."' onclick='redirect();' style='width:90px;height:45px;font-size:1.2em;";
		...
		<?php if((empty($table))&&(empty($vt))) echo "<a href='servir.php?menuParent=".$_SESSION['menuParenT']."&vt=1&tk=1'   id='lien1' style='text-decoration:none;'></a>";
	 		...
	 		echo "</span>"; if((empty($table))&&(empty($vt))) echo "<span style='float:left;font-weight:bold;'><a href='servir.php?menuParent=".$_SESSION['menuParenT']."&vt=1&tk=1' id='lien1' class='info' style='border-radius: 5px;-moz-border-radius: 5px;-webkit-border-radius: 5px;background-color: gold;color:#444739;font-style:italic;text-decoration:none;'> Takeaway&nbsp;</a> </span>
			<span style='float:right;font-weight:bold;'><a href='' onclick='edition2();return false;'  id='lien1' class='info' style='background-color: gold;color:#444739;font-style:italic;text-decoration:none;border-radius: 5px;-moz-border-radius: 5px;-webkit-border-radius: 5px;'><span style='font-size:0.7em;'></span> Ajouter un(e) serveur(se)</a> </span>";
		?>
		<h2 style='font-size:1.1em;color:maroon;font-family:cambria;'>&nbsp;&nbsp;  </h2>

				<?php if((!empty($table))||(!empty($vt))||(!empty($tk)))
					{include('serv.php'); //header('Location: servir.php');
					}
