...
function FancyTable($header, $data)
{
    ...
    {  	$this->SetFont('times','B',7);
        ...
        $this->Cell($w[1],5,utf8_decode($row[1]),'LR',0,'C',$fill);
		$this->Cell($w[2],5,$row[2],'LR',0,'C',$fill);
        $this->Cell($w[3],5,number_format($row[3],0,',',' '),'LR',0,'C',$fill);
		$this->Cell($w[4],5,number_format($row[4],4,',',' '),'LR',0,'C',$fill);
		$this->Cell($w[5],5,number_format($row[5],0,',',' '),'LR',0,'C',$fill);
		$this->Cell($w[6],5,number_format($row[6],4,',',' '),'LR',0,'C',$fill);
...
$pdf = new PDF();
// Titres des colonnes
$header = array('N° Client', 'Nom & Prénoms', 'Chambre', 'Nuité', 'Montant HT','Taxe','Montant TTC');
// Chargement des données
