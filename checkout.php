<?php
	...
	if(isset($_POST['LIBERER']) && !empty($_POST['choix'])){ //echo 125;
		...
		/* 	if($i==1)
				...
				$s=mysqli_query($con,"SELECT * FROM location WHERE numfiche='".$numfiche."'");$nbre_result=mysqli_num_rows($s);
 			   if($nbre_result>0)
			   {    $sql="UPDATE location SET datsortie='".$date."', heuresortie='".$heure."', etatsortie='OUI' WHERE numfiche='".$numfiche."'";
				 ...
				 //$fet2=mysqli_query($con,"UPDATE mensuel_fiche1 SET datsortie='".$date."', heuresortie='".$heure."', etatsortie='OUI' WHERE numfiche='".$numfiche."'");
					 ...
					 $fet2=mysqli_query($con,"UPDATE location SET datsortie='".$date."', heuresortie='".$heure."', etatsortie='OUI' WHERE numfiche='".$numfiche."'");
				}
					...
					$due = $mt-$var1;
					$N_reel=$n-$rat['np'] ;
			 $update = "UPDATE compte1 SET  due='".$due."',N_reel='".$N_reel."' WHERE CONVERT( `compte1`.`numfiche` USING utf8 ) = '".$numfiche."' AND CONVERT( `compte1`.`numsalle` USING utf8 ) = '$numsalle' LIMIT 1" ;
		     $fet1=mysqli_query($con,$update);
		    //$fet2=mysqli_query($con,"UPDATE location SET  due='".$due."',N_reel='".$N_reel."' WHERE CONVERT( `location`.`numfiche` USING utf8 ) = '".$numfiche."' AND CONVERT( `location`.`numch` USING utf8 ) = '$numch' LIMIT 1" );
				...
				echo '<meta http-equiv="refresh" content="1; url=checkout.php?menuParent='.$_SESSION['menuParenT'].'" />';
			//}
		}
			...
			echo "<script language='javascript'>";
			echo 'alertify.success("Salle(s) libérée(s) avec succès !");';
			echo "</script>";
	}
								...
								else
								{
									...
									$bgcouleur = "#dfeef3";
										...
										echo"<tr class='rouge1' bgcolor='".$bgcouleur."'>";?>
											...
											echo"<td align='center'";  if($n>=15) echo " style=''"; echo">"; echo "&nbsp;".substr($Jour_actuel,8,2).'-'.substr($Jour_actuel,5,2).'-'.substr($Jour_actuel,0,4); echo"</td>";
											//if($sal!=1)
											if($_SESSION['poste']=="agent")
