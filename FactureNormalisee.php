...
/* 		if(isset($_SESSION['remise'])&& ($_SESSION['remise']>0))
		...
		//if($_SESSION['Mtotal']<=$_SESSION['avanceA'])
			...
			if(isset($_POST['"button_checkbox_2"']))  $difference = - $_SESSION['Mtotal'];
		//if(isset($_SESSION['modulo']) && ($_SESSION['modulo'] > 0))

		$Mtotal=$_SESSION['Mtotal'];
		
		//if(isset($_SESSION['button_checkbox_2'])) $_SESSION['remise'] = 0;
		...
		if($Mtotal<$_SESSION['avanceA'])
			...
			//if(isset($_POST['"button_checkbox_2"']))  $difference = 0;
		...
		if(isset($_SESSION['remise'])&& ($_SESSION['remise']>0)) $Mtotal = $_SESSION['avanceA'] ; //A revoir
				...
				//if(isset($_POST['"button_checkbox_2"']))  $Mtotal = 0;
	...
	$this->Rect(5, 275, 200, 15, "D");
          ...
          //if(isset($_SESSION['reimprime']))
			  if(isset($_SESSION['button_checkbox_2']))
					$this->Cell( $this->GetPageWidth(), 7, "NB: La présente facture n'est pas encore soldée. Elle sera normalisée une fois règlement effectué. Merci pour votre compréhension! ", 0, 0, 'L');
				else 
...
function FancyTable($header, $data)
{
	...
	$taux_tva =12;
        ...
        {   $Xo=0; $Yo=0; if(isset($_SESSION['Apply_AIB'])) { $Xo+=6; $Yo+=6; }
            ...
            $this->SetLineWidth(0.1); $this->Rect(135, $y+10, 70, 36+$Yo, "D");
            // les traits verticaux
            $this->Line(165+7, $y+10, 158+7+7, $y+10+36+$Yo); //$this->Line(164, 221, 164, 245);
			//$this->Line(181, 221, 181, 245);
            // les traits horizontaux pas de 6 et demarre a 221
            $this->Line(135, $y+16, 205, $y+16);
			$this->Line(135, $y+16+6, 205, $y+16+6);
			$this->Line(135, $y+16+6+6, 205, $y+16+6+6);
			if(isset($_SESSION['Apply_AIB'])) $this->Line(135, $y+16+6+6+$Xo, 205, $y+16+6+6+$Yo);
			$this->Line(135, $y+16+6+6+6+$Xo, 205, $y+16+6+6+6+$Yo);
			...
			$TotalTva=round(0.18*$TotalHT); $TotalHT=round($TotalHT);
			...
			$this->SetXY( 175,$y+10 ); $this->Cell( 24, 6,$TotalHT." ".$devise, 0, 0, 'R');
			
			//if($voir!=1)
			$this->SetXY( 175,$y+10+6 ); $this->Cell( 24, 6, $TotalTva." ".$devise, 0, 0, 'R');
			
			$this->SetXY( 175,$y+10+6+6 ); $this->Cell( 24, 6, $TotalTaxe." ".$devise, 0, 0, 'R');
			if(isset($_SESSION['Apply_AIB'])&&!empty($_SESSION['Apply_AIB'])) 
				{$this->SetXY( 175, $y+10+6+6+$Yo ); $this->Cell( 24, 6, $ValAIB." ".$devise, 0, 0, 'R');	}		
			$this->SetXY( 175,$y+10+6+6+6+$Yo ); $this->Cell( 24, 6, $TotalTTC." ".$devise, 0, 0, 'R');
			$this->SetXY( 175,$y+10+6+6+6+6+$Yo ); $this->Cell( 24, 6, $remise." ".$devise, 0, 0, 'R');
			$this->SetXY( 175,$y+10+6+6+6+6+6+$Yo ); $this->Cell( 24, 6, $NetPayer." ".$devise, 0, 0, 'R');
            $this->SetFont('Arial','',8);
            $this->SetXY( 143, $y+10 ); $this->Cell( 25, 6, "Montant HT ", 0, 0, 'R');
			...
			if(isset($_SESSION['Apply_AIB'])&&!empty($_SESSION['Apply_AIB'])) 
				{$this->SetXY( 143, $y+10+6+6+$Yo ); $this->Cell( 25, 6, "AIB ( ".$PourcentAIB." % )", 0, 0, 'R');	}		
            $this->SetXY( 143, $y+10+6+6+6+$Yo ); $this->Cell( 25, 6, "Total TTC", 0, 0, 'R');
			$this->SetXY( 143, $y+10+6+6+6+6+$Yo ); $this->Cell( 25, 6, "Remise accordée ".$end, 0, 0, 'R');
			$this->SetXY( 143, $y+10+6+6+6+6+6+$Yo ); $this->Cell( 25, 6, "Net à payer ", 0, 0, 'R');
