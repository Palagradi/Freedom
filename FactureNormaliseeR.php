...
{ /* 	function temporaire( $texte )
	...
	// $this->SetXY( $x, $y+75 );
		...
		if($_SESSION['avanceA']>=0)
			$this->fact_dev("Facture N°: ",$_SESSION['initial_fiche']); 
		else 
			$this->fact_dev("Facture d'avoir N°: ",$_SESSION['initial_fiche']); 
...
/* 		if(isset($_SESSION['remise'])&& ($_SESSION['remise']>0))
		...
		if($Mtotal<$_SESSION['avanceA'])
			...
			if(isset($_SESSION['remise'])&& ($_SESSION['remise']>0))
				{//$titre="REMISE"; 
				$Mtotal+=$_SESSION['remise'] ; 
				//$difference=$_SESSION['remise']; 
				}
			//else
				//$titre="SOLDE";
		...
		$modeReglement="Espèces";//$this->addClient($modeReglement); 
		...
		if($_SESSION['avanceA']>=0)
			$this->Cell( 10, 8, "Mode de règlement : ".$modeReglement, 0, 0, 'C');
		else 
			{$this->Cell( 47, 8, "Ref. de facture originale : ".$_SESSION['Foriginal1']." (".$_SESSION['Foriginal2'].")", 0, 0, 'C');
			$this->Cell( 100, 8, "|    Mode de règlement : ".$modeReglement, 0, 0, 'C'); 
			}


		$this->SetTextColor(0,0,0);$code=$_SESSION['userId'];
		$this->addReglement($code,$_SESSION['vendeur']);
	...
	$this->Rect(5, 275, 200, 15, "D");
          ...
          //if(isset($_SESSION['reimprime']))
			//			$this->Cell( $this->GetPageWidth(), 7, "NB: Cette facture n'est qu'un duplicata. L'original a été éditée le : ".substr($_SESSION['date_emission'],8,2).'-'.substr($_SESSION['date_emission'],5,2).'-'.substr($_SESSION['date_emission'],0,4), 0, 0, 'L');
		 //else 
			 $this->Cell( $this->GetPageWidth(), 7, " Merci pour votre visite! Confiants que vous avez passé un bon séjour dans notre hôtel, nous espérons vous revoir de sitôt.", 0, 0, 'L');
/*
        $y1 = 270;
...
function FancyTable($header, $data)
{
    ...
    $this->Cell(array_sum($w),0,'','T'); $x= $this->GetX();$y=$this->GetY();
           ...
           // $this->SetFont('Arial','B',8); $this->SetXY( 181, 227 ); $this->Cell( 24, 6, number_format($_SESSION['TotalHT'], 2, ',', ' '), 0, 0, 'R');
			...
			//$this->SetXY( 45, $y+10 ); $this->Cell( 38, 5, "Mode de règlement : ", 0, 0, 'L'); $this->Cell( 38, 5, " Espèces ", 0, 0, 'L');
	...
	$taux_tva =12;
        ...
        {   $Xo=0; $Yo=0; if(isset($_SESSION['Apply_AIB'])) { $Xo+=6; $Yo+=6; }
            ...
            $this->SetLineWidth(0.1); $this->Rect(135, $y+10, 70, 40+$Yo, "D");
            // les traits verticaux
            $this->Line(165+7, $y+10, 158+7+7, $y+15+35+$Yo); //$this->Line(164, 221, 164, 245);
			//$this->Line(181, 221, 181, 245);
            // les traits horizontaux pas de 6 et demarre a 221
            $this->Line(135, $y+15.5, 205, $y+15.5);
			$this->Line(135, $y+15+6, 205, $y+15+6);
			$this->Line(135, $y+14.5+6+6, 205, $y+14.5+6+6);
			if(isset($_SESSION['Apply_AIB'])) $this->Line(135, $y+14.5+6+6+$Xo, 205, $y+14.5+6+6+$Yo);
			$this->Line(135, $y+14.5+6+6+6+$Xo, 205, $y+14.5+6+6+6+$Yo);
			$this->Line(135, $y+14.5+6+6+6+6+$Xo, 205, $y+14.5+6+6+6+6+$Yo);
			$this->Line(135, $y+14.5+6+6+6+6+6+$Xo, 205, $y+14.5+6+6+6+6+6+$Yo);
			...
			$TotalTTC=isset($_SESSION['Mtotalr'])?$_SESSION['Mtotalr']:$TotalTTC;
			//$TotalTTC+=$remise;
			...
			$NetC = $TotalHT - $remise;
			$TotalTva=round(0.18*$NetC);
			$NetPayer=$NetC+$TotalTva+$TotalTaxe;
			$end=isset($_SESSION['end'])?$_SESSION['end']:0;
						
			
			$voir=1; 

            $this->SetFont('Arial','',8);
			$this->SetXY( 175,$y+10 ); $this->Cell( 24, 6,$TotalTTC." ".$devise, 0, 0, 'R');
			
			//if($voir!=1)
			$this->SetXY( 175,$y+10+6 ); $this->Cell( 24, 6, $TotalHT." ".$devise, 0, 0, 'R');  //remise; 
			
			$this->SetXY( 175,$y+10+6+5.25 ); $this->Cell( 24, 6, "- ".$remise." ".$devise, 0, 0, 'R');
			if(isset($_SESSION['Apply_AIB'])&&!empty($_SESSION['Apply_AIB'])) 
				{$this->SetXY( 175, $y+10+6+5+$Yo ); $this->Cell( 24, 6, $ValAIB." ".$devise, 0, 0, 'R');	}		
			$this->SetXY( 175,$y+10+6+6+5.25+$Yo ); $this->Cell( 24, 6, $NetC." ".$devise, 0, 0, 'R');
			$this->SetXY( 175,$y+10+6+6+6+5+$Yo ); $this->Cell( 24, 6, $TotalTva." ".$devise, 0, 0, 'R');
			$this->SetXY( 175,$y+10+6+6+6+6+4.5+$Yo ); $this->Cell( 24, 6, $TotalTaxe." ".$devise, 0, 0, 'R');
			$this->SetXY( 175,$y+10+6+6+6+6+6+4.5+$Yo ); $this->Cell( 24, 6, $NetPayer." ".$devise, 0, 0, 'R');
			
			//$end =""; 
			//if(isset($_SESSION['end'])&&($_SESSION['end']>0)) $end = $_SESSION['end']; 
			
            $this->SetFont('Arial','',8);
            $this->SetXY( 143, $y+10 ); $this->Cell( 25, 6, "Montant TTC", 0, 0, 'R');		
			//if($voir!=1)	
			$this->SetXY( 143, $y+10+6 ); $this->Cell( 25, 6, " Montant HT" , 0, 0, 'R');
			
            $this->SetXY( 143, $y+10+6+5.25 ); $this->Cell( 25, 6, "Remise accordée ".$end, 0, 0, 'R');
			//$this->SetXY( 143, $y+10+6+6 ); $this->Cell( 25, 6, "Taxe de séjour (F) ", 0, 0, 'R');
			if(isset($_SESSION['Apply_AIB'])&&!empty($_SESSION['Apply_AIB'])) 
				{$this->SetXY( 143, $y+10+6+5.25+$Yo ); $this->Cell( 25, 6, "AIB ( ".$PourcentAIB." % )", 0, 0, 'R');	}		
            $this->SetXY( 143, $y+10+6+6+5+$Yo ); $this->Cell( 25, 6, "Net commercial", 0, 0, 'R');
			  //$this->SetXY( 143, $y+10+6+6+6+$Yo ); $this->Cell( 25, 6, "Total TTC", 0, 0, 'R');
			$this->SetXY( 143, $y+10+6+6+6+5.25+$Yo ); $this->Cell( 25, 6, "TVA Collectée", 0, 0, 'R');
			$this->SetXY( 143, $y+10+6+6+6+6+5+$Yo ); $this->Cell( 25, 6, "Taxe de séjour (F) ", 0, 0, 'R');
			$this->SetXY( 143, $y+10+6+6+6+6+6+4.5+$Yo ); $this->Cell( 25, 6, "Net à payer ", 0, 0, 'R');
...
$this->SetXY( $r1+2, $y1+4 ); $this->SetTextColor (45); $this->Cell( 47, 8, "ADRESSE DE FACTURATION", 0, 0, 'C');
	...
	//$this->SetDash(2,1);  //Soulignement en pointillé
    $this->SetLineWidth(.1);
...
//$this->Line(20,20,190,20);
//$this->SetFont('times','B',10);
$this->SetXY( $r1+14, $y1+4 ); $this->SetTextColor (45); $this->Cell( 30, 8, "dddd", 0, 0, 'C');
$this->SetFont('Arial','',10);
...
// Titres des colonnes
$header = array('N°', 'Nom & Prénoms', 'Désignation', 'Prix Unitaire', 'Nuitée','Montant TTC');
// Chargement des donn?es
...
if($_SESSION['avanceA']>=0) $val=$_SESSION['Mtotal'];  else $val=0;
