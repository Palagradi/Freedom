...
$result =   mysqli_query($con, 'SELECT * FROM boisson,config_boisson,conditionnement,QteBoisson WHERE QteStock>0 AND QteBoisson.id=boisson.Qte AND conditionnement.id=boisson.Conditionne AND config_boisson.id=boisson.Categorie AND pc=0 AND  Depot = \'2\' AND (LibCateg LIKE \'%' . safe( $_GET['qt'] ) . '%\'  OR designation LIKE \'%' . safe( $_GET['qt'] ) . '%\' OR LibConditionne LIKE \'%' . safe( $_GET['qt'] ) . '%\'
    ...
    ?>
		 	<tr class='rouge1' bgcolor=' <?=$data->QteStock<=0?"#D2B48C":$bgcouleur; ?>'>
			  <td align='center' style='padding:7px;border-right: 2px solid #ffffff; border-top: 2px solid #ffffff'><?php echo $j; ?>.</td>
				...
				<td align='center'  style='border-right: 2px solid #ffffff; border-top: 2px solid #ffffff'> <?php echo $data->PrixUnitaire; ?></td>
