	...
	{  	if($this->PageNo()==1)
		...
		if(isset($_SESSION['logo'])) 
			//$this->Image($_SESSION['logo'],10,15,30,20);
			 $this->Image($_SESSION['logo'],7,7,45,47);   //STELLA MARINS
...
/* 		if(isset($_SESSION['remise'])&& ($_SESSION['remise']>0))
		...
		if(($Mtotal< $_SESSION['avanceA'])&& (isset($_SESSION['modulo'])&&($_SESSION['modulo']>0)))  $Mtotal = $_SESSION['avanceA'] - $_SESSION['modulo'];
	...
	$this->Rect(5, 271, 200, 15, "D");
          ...
          //if(isset($_SESSION['reimprime']))
			  if(isset($_SESSION['button_checkbox_2']))
					$this->Cell( $this->GetPageWidth(), 7, "NB: La présente facture n'est pas encore soldée. ", 0, 0, 'L');
				else 
...
//Positionnement ? 1,5 cm du bas
		...
		$this->Cell(-200,25,'République du Bénin ',0,0,'C');
...
function FancyTable($header, $data)
{
	...
	$taux_tva =12;
        ...
        {   $Xo=0; $Yo=0; if(isset($_SESSION['Apply_AIB'])) { $Xo+=6; $Yo+=6; }
			...
			if(isset($_SESSION['Vente']) && ($_SESSION['Vente']==1)) $taxeV="Taxe spécifique"; else $taxeV="Taxe de séjour (F) ";;
			...
			$this->SetXY( 143, $y+12+6+6+6+$Yo ); $this->Cell( 25, 6, "Remise accordée ".$end, 0, 0, 'R');
...
//if($_SESSION['objet']=="Location de salle")	$unite="Qté"; else $unite= 'Nuitée'; 
...
// Titres des colonnes
//$header = array('N°', 'Nom & Prénoms', 'Désignation', 'Prix Unitaire',$unite,'Montant TTC');
// Chargement des donn?es
...
$name="Mme Monique BADOU";$img="logo\signature.png"; $montant=$params1."".$params2; $signataire="La Directrice";
...
unset($_SESSION['view']); unset($_SESSION['Vente']);unset($_SESSION['reference']);
?>
