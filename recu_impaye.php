<?php
	...
	$s=mysql_query("SELECT * FROM fiche1, compte,client WHERE fiche1.numfiche=compte.numfiche and client.numcli=fiche1.numcli and fiche1.numfiche='".$_SESSION['numfich']."'");
			...
			<tr>
				<td>
					...
					if ($n==0){$n= $n+1;}
								...
								$date=substr($ret1['datarriv'],8,2).'/'.substr($ret1['datarriv'],5,2).'/'.substr($ret1['datarriv'],0,4); 
							$date2=$ret1['date'];
							if(!empty($date2))
								echo $dat=$date2;	
							else
							...
							?>  au 
							<?php 
							    $date1=substr($dat,5,4); $date2=substr($dat,3,2); $date3=substr($dat,0,2);
...
<?php 
 ...
 $tu=mysql_query("INSERT INTO reedition_facture VALUES ('".date('Y-m-d')."','$Heure_actuelle','".$_SESSION['login']."','$num_recu','$num_recu','".addslashes($_SESSION['nomclient'])."','Hebergement','".$_SESSION['date']."','$d','$taxe','$tva','$var','".$_SESSION['remise']."','".$_SESSION['somme_paye']."')");
 $tu=mysql_query("INSERT INTO reeditionfacture2 VALUES ('$num_recu','Chambre $nomch','".$_SESSION['tch']."','".$_SESSION['occup']."','".$_SESSION['tarif1']."','".$_SESSION['nuite']."','".$_SESSION['tarif1']*$_SESSION['nuite']."')");
?>