...
$i = 1;
 $moisX=array("");
 $req9 = mysqli_query($con,"SELECT nomch FROM chambre ORDER BY numch ");
...
$titre="Planning de Réservation des Chambres de <span style='color:green;'>".$nomHotel."</span> pour le mois de";
...
<table cellpadding='0' cellspacing='0' style="width:100%;margin: auto;border:1px solid black;border-collapse:collapse;box-shadow: 10px 10px 5px #888888;">
	...
	<a href="<?php if(!empty($_GET['mois'])) echo "?mois=".$_GET['mois']; ?>&annee=<?php echo $annee-1; ?>" style="font-size:50%;vertical-align:middle;text-decoration:none;"><?php echo $annee-1; ?></a> <?php echo $annee; ?> <a href="<?php if(!empty($_GET['mois'])) echo "?mois=".$_GET['mois']; ?>&annee=<?php echo $annee+1; ?>" style="font-size:50%;vertical-align:middle;text-decoration:none;"><?php echo $annee+1; ?>
		...
		<span style='display:block;float:right;right:0px;font-size:75%;'>
		...
		<tr style=''>
			<th style="width: 35pt;<?php echo $StyleTh; ?>background:black"><?php echo "Liste des Chambres"; ?></th>
          <?php
		   	$m=$mois;
		    for($jour=1;$jour<=$NbrDeJour[$mois];$jour++){
					?>
					<th style="width: 38pt;<?php echo $StyleTh; ?>background:#FF9933"><?php echo $jour; ?></th>
				<?php
				} ?>
		 </tr>
</table>
...
<?php
 $jour-=1; ;
 //echo " http://www.solucior.com/fr/13-Scroll_div_avec_javascript.html";

$req8 = mysqli_query($con,"DELETE FROM reserverch where numsalle='0' ");
$Recordset = mysqli_query($con,"DELETE FROM reservationch  WHERE numch='0'");
$query_Recordset1 = "SELECT * FROM reservationch,chambre where reservationch.numch=chambre.numch AND chambre.EtatChambre='active' AND annee like $annee AND mois like $mois  ORDER BY position ASC";
...
while($row_Recordset1 = mysqli_fetch_array($Recordset1)){
	$ListeReserv[$cpt][0] = $row_Recordset1['nomch'];
...
$Recordset1 = mysqli_query($con,"CREATE OR REPLACE VIEW reservation AS SELECT chambre.nomch,reservationch.debut,reservationch.fin,reservationch.position,reservationch.prenomdemch,reservationch.nomdemch,reservationch.codegrpe,reservationch.contactdemch,reservationch.avancerc  AS avance FROM reservationch,chambre where reservationch.numch=chambre.numch AND chambre.EtatChambre='active' AND annee like $annee AND mois like $mois  ORDER BY position ASC") or die(mysqli_error($con));
...
<table cellpadding='0' cellspacing='0' style='border-collapse: collapse;'>";
		for($i=1;$i < $totalRows_Recordset1;$i++) {
			$JourReserve=0;
				echo "<tr style=''>
					<td style='background:#1A1D26;border:1px solid white;color:white;width:68px;'> &nbsp;".ucfirst($moisX[$i])."</td>";
 ...
 /* 						for($j=0;$j<count($ListeReserv);$j++) {
						...
						$query_Recordset1 = "SELECT * FROM reservation where nomch='".$moisX[$i]."' AND '".$mois."' BETWEEN debut AND fin";
						$Recordset1 = mysqli_query($con,$query_Recordset1) or die(mysqli_error($con));
              ...
              if(strlen($m)==1) $m="0".$m;
							...
							$demandeur = !empty($data['codegrpe'])?$data['codegrpe']:$data['nomdemch']." ".$data['prenomdemch'];
              $demandeur .=" | <font color='#5C110F'><u>Contact </u>: </font>".$data['contactdemch'];  if($data['avance']>0) {$demandeur.=" | <font color='#5C110F'><u>Avance payée</u> :</font> ".$data['avance']." ".$devise;$backgd="#4F8037";} else $backgd="#C82605"; $color=$backgd;
            } //#5C110f; #C82605
							echo "<table  cellpadding='0' cellspacing='0' style='border:0px solid black;'>
								<tr>
									<td style='color:#FF9933;background:".$backgd."; width: 35pt;height: 10pt;'>
									<a class='info2' href='#' style='text-decoration:none;color:white;' title=''> <span style='font-size:0.9em;font-weight:bold;color:red;'>
									<font color='#5C110F'><u>Client </u>:</font> ".$demandeur."<br/><span style='color:#5C110F;'> Période d'occupation envisagée : ".$periode."</span></span>
									<font style='color:".$color.";'>p</font>
                  </a>
									</td>
								</tr>
						</table>";
