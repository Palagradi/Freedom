		...
		{	//		 if(isset($_POST['cat'])) 
			...
			if($reqsel)
				{	//echo "<script language='javascript'>";
					...
					$query_Recordset1 = "SELECT boissonb.numero AS numero,operation.numero AS Numop1,numero2,designation,Qte_initiale,Qte_entree,qte_finale,Qte_Stock,date_modification,heure_operation,Qte,Seuil,StockReel FROM operation,boissonb WHERE boissonb.numero2=operation.reference1  AND date_modification BETWEEN '".$debutN."'	AND '".$finN."' ";
					//echo '<meta http-equiv="refresh" content="1; url=InventairePermanent.php?menuParent=Restauration" />';
						...
						if(isset($idr))
						    $query_Recordset1 = "SELECT boissonb.numero AS numero,operation.numero AS Numop1,numero2,designation,Qte_initiale,Qte_entree,Qte_Stock,qte_finale,date_modification,heure_operation,Qte,Seuil,StockReel FROM operation,boissonb WHERE boissonb.numero2=operation.reference1 
						 AND Categorie = '".$idr."' AND date_modification BETWEEN '".$debutN."'	AND '".$finN."'	AND operation.numero IN (SELECT MAX(operation.numero)  FROM operation,boissonb WHERE boissonb.numero2=operation.reference1  
						 AND date_modification BETWEEN '".$debutN."' AND '".$finN."' AND designation_operation <> 'Mise à jour Produit alimentaire' GROUP BY  designation,Qte  ORDER BY operation.numero) ";
							...
							if(isset($_GET['checkbox']))
								...
								{$query_Recordset1 = "SELECT boissonb.numero AS numero,operation.numero AS Numop1,numero2,designation,Qte_initiale,Qte_entree,Qte_Stock,qte_finale,date_modification,heure_operation,Qte,Seuil,StockReel
								FROM operation,boissonb WHERE boissonb.numero2=operation.reference1  AND operation.numero IN (SELECT MAX(operation.numero)  FROM operation,boissonb WHERE boissonb.numero2=operation.reference1  
								AND date_modification BETWEEN '".$debutN."'	AND '".$finN."' AND designation_operation <> 'Mise à jour Produit alimentaire' GROUP BY  designation,Qte  ORDER BY operation.numero)";
								}
							}
				}
		...
		<tr style=''>
			...
			<?php
					...
					{$i++;
						...
						FROM factureResto,tableencours,boissonb  WHERE  factureResto.numTable=tableencours.numTable AND tableencours.LigneCde=boissonb.designation AND  tableencours.QteInd=boissonb.Qte  AND date_emission BETWEEN '".$debutN."'	AND '".$finN."'  AND boissonb.numero2='".$data->numero2."'  ";
