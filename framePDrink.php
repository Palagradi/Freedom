...
if(($numero>0)&&($Qte>0)){echo "&nbsp;";
	...
	if(!empty($RegimeTVA)&&($RegimeTVA>0))  $tva=0 ; else $tva=round($data['PrixPack']/(1+$TvaD)*$TvaD);
	...
	{	echo "<script src='js/sweetalert.min.js'></script>";
		...
		if(substr($data['Libellepc'],0,1)=="P") $pack="[Pack de ".$data['qtepc']."]"; else if(substr($data['Libellepc'],0,1)=="C") $pack="[Casier de ".$data['qtepc']."]";else $pack="";
		...
		if($tk==0)
			...
			$rk="SELECT * FROM tableEnCours WHERE LigneCde='".$designation."' AND LigneType='0' AND QteInd ='".$LibQte."' AND numTable='".$table."' AND numTk='".$tk."' AND created_at='".$Jour_actuel."' AND Etat <> 'Desactive'";
...
/* 		function JSalertQte(param,param2){
		swal("QUANTITE COMMANDEE",{
		  content: {
			element: "input",
			attributes: {
			  placeholder: "Saisissez la quantité commandée ici",
			  type: "number",
			  min : "1",
			},
		  },
			...
			.then((value) => {
				//var numero = param; 
				document.location.href='framePDrink.php?Qte='+value+'&numero='+param+'&tk='+param2;
			});
		} */
    ...
    .then(() => {
        ...
        if (!quantity) {
            ...
            document.location.href = `framePDrink.php?Qte=${quantity}&status=${encodeURIComponent(selectedStatus)}&numero=${param}&tk=${param2}`;
        }
    });
}
		</script>
...
<?php
	...
	$result=mysqli_query($con,"SELECT * FROM boisson,config_boisson,conditionnement,QteBoisson,casier WHERE QteBoisson.id=boisson.Qte AND conditionnement.id=boisson.Conditionne AND config_boisson.id=boisson.Categorie AND casier.id=boisson.pc AND pc<>0 AND Depot = '2' ORDER BY QteStock DESC,PrixPack DESC");
	$cpteur=1;$i=0;$j=0;
    ...
    { $j++;
		if($cpteur == 1)
			...
			else
			{
				$cpteur = 1;
				$bgcouleur = "#dfeef3";
			}  $i++;  if($i%2==0){$color="#FC7F3C";$plus="add4"; }else {$color="red";$plus="add5";}
