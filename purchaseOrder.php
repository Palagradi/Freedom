...
//
    require('fpdf2/fpdf.php');  include 'connexion.php';   $logo_societe='logo/Sesy.png'; $TypeP='Economat'; $reglement="Cheque";
    ...
    $var_id_facture = isset($_GET['id_param'])?$_GET['id_param']:$id_param; $explore = explode('_',$var_id_facture); $dateCde=$explore[0]; $NumFrs=$explore[1];
    ...
    $sql = "select count(*) FROM boncommande where Date='".$dateCde."' AND NumFrs='".$NumFrs."'";
    ...
    While ($num_page <= $nb_page)
    {
        ...
        $pdf->Image($logo_societe, 10, 10, 80, 55);

        // n° page en haute à droite
        $pdf->SetXY( 120, 5 ); $pdf->SetFont( "Arial", "B", 12 ); $pdf->Cell( 160, 8, $num_page . '/' . $nb_page, 0, 0, 'C');

		$select ="SELECT SUM(PrixFrs) AS PrixFrs FROM produits,fournisseurs,boncommande WHERE boncommande.NumPro=produits.Num  AND fournisseurs.NumFrs=produits.Fournisseur AND produits.Type='".$TypeP."' AND boncommande.Date='".$dateCde."'";
        $result = mysqli_query($con, $select)  or die ('Erreur SQL : ' .$select .mysqli_connect_error() );
        $data = mysqli_fetch_object($result); $somme=$data->PrixFrs; mysqli_free_result($result);
		...
		$select ="SELECT produits.Designation AS designation,Qte_Stock,PrixFrs,RaisonSociale,boncommande.QteCde AS QteCde,boncommande.NumFrs  FROM produits,fournisseurs,boncommande WHERE boncommande.NumPro=produits.Num AND fournisseurs.NumFrs=produits.Fournisseur AND produits.Type='".$TypeP."' AND boncommande.Date='".$dateCde."'";
        $result = mysqli_query($con, $select)  or die ('Erreur SQL : ' .$select .mysqli_connect_error() );
        $row = mysqli_fetch_row($result);  mysqli_free_result($result); //$NumFrs=$row['NumFrs'];

        $champ_date = date_create($dateCde); $annee = date_format($champ_date, 'Y');
        ...
        // date facture
        $champ_date = date_create($dateCde); $date_fact = date_format($champ_date, 'd/m/Y');
        ...
        if ($num_page == $nb_page)
        {
            ...
            $nombre_format_francais = "Total HT : " . number_format($somme, 2, ',', ' ') . " €";
            $pdf->SetFont('Arial','',10); $pdf->SetXY( 95, 213 ); $pdf->Cell( 63, 8, $nombre_format_francais, 0, 0, 'C');
            // en bas à droite
            $pdf->SetFont('Arial','B',8); $pdf->SetXY( 181, 227 ); $pdf->Cell( 24, 6, number_format($somme, 2, ',', ' '), 0, 0, 'R');

            // trait vertical cadre totaux, 8 de hauteur -> 213 + 8 = 221
            $pdf->Rect(5, 213, 200, 8, "D"); $pdf->Line(95, 213, 95, 221); $pdf->Line(158, 213, 158, 221);

            // reglement
            $pdf->SetXY( 5, 225 ); $pdf->Cell( 38, 5, "Mode de Règlement :", 0, 0, 'R'); $pdf->Cell( 55, 5, $reglement, 0, 0, 'L');
            // echeance
        ...
        $select = "select RaisonSociale,Adresse,Telephone,Email from fournisseurs where NumFrs=" . $NumFrs;
		$result = mysqli_query($con, $select)  or die ('Erreur SQL : ' .$select .mysqli_connect_error() );
        $row_client = mysqli_fetch_row($result);
        mysqli_free_result($result);
        ...
        if ($row_client[1]) { $pdf->SetXY( $x, $y ); $pdf->Cell( 100, 8, $row_client[2], 0, 0, ''); $y += 4;}
        if ($row_client[1]) { $pdf->SetXY( $x, $y ); $pdf->Cell( 100, 8, $row_client[3], 0, 0, ''); $y += 4;}
        if ($row_client[2] || $row_client[3]) { $pdf->SetXY( $x, $y ); $pdf->Cell( 100, 8, $row_client[3] . ' ' .$row_client[3] , 0, 0, ''); $y += 4;}
        if ($row_client[3]) { $pdf->SetXY( $x, $y ); $pdf->Cell( 100, 8, 'N° TVA Intra : ' . $row_client[3], 0, 0, '');}
		...
		$sql ="SELECT produits.Designation AS libelle,Qte_Stock AS qte,PrixFrs AS pu,RaisonSociale,boncommande.QteCde AS QteCde,boncommande.NumFrs  FROM produits,fournisseurs,boncommande WHERE boncommande.NumPro=produits.Num AND fournisseurs.NumFrs=produits.Fournisseur AND produits.Type='".$TypeP."' AND boncommande.Date='".$dateCde."'";
        ...
        while ($data =  mysqli_fetch_assoc($res))
        {
            ...
            // Taux
            $nombre_format_francais = number_format($taux_tva, 2, ',', ' ');
