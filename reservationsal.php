<?php
include_once 'menu.php'; $sal=!empty($_GET['sal'])?$_GET['sal']:NULL;
if(empty($sal))
	...
	unset($_SESSION['exhonerer1']);unset($_SESSION['exhonerer2']);unset($_SESSION['exhonerer_aib']);unset( $_SESSION['remise']);  }
	else $_SESSION['nbsal1']=$sal;
	
	unset($_SESSION['chambre']);	//unset($_SESSION['debutS']); 	unset($_SESSION['debutS']);
...
// enregistrement dans reservationch
	if (isset($_POST['ok'])&& $_POST['ok']=='VALIDER')
		{  	//if(!empty($_POST['edit3'])&&!empty($_POST['edit4'])&&!empty($_POST['edit5'])&&!empty($_POST['ladate'])&&!empty($_POST['ladate2'])&&(!empty($_POST['edit6'])||!empty($_POST['edit12'])))
			...
			//echo $_SESSION['nuite'];
			$_SESSION['debutS']=$debut; $_SESSION['dat1']=$fin;	$_SESSION['finS']=$fin;
			$_SESSION['numresch']=!empty($_POST['edit1'])?$_POST['edit1']:NULL;
			$_SESSION['date_du_jour']=!empty($_POST['edit2'])?$_POST['edit2']:NULL;
			$_SESSION['contact']=!empty($_POST['edit5'])?$_POST['edit5']:$_SESSION['contact'];
			$_SESSION['groupe']=!empty($_POST['combo1'])?$_POST['combo1']:$_SESSION['groupe'];
			//$_SESSION['nomT']=!empty($_POST['edit3'])?$_POST['edit3']:$_SESSION['nom'];
			//$_SESSION['prenom']=!empty($_POST['edit4'])?$_POST['edit4']:$_SESSION['prenom'];
			$_SESSION['nomT']=$_POST['edit3'];$_SESSION['prenomT']=$_POST['edit4'];
			//$_SESSION['nbch']=!empty($_POST['edit6'])?$_POST['edit6']:0;
			$_SESSION['nbsal']=!empty($_POST['edit6'])?$_POST['edit6']:0;
			//$_SESSION['nbch1']=!empty($_POST['edit6'])?$_POST['edit6']:0;
			$_SESSION['nbsal1']=!empty($_POST['edit6'])?$_POST['edit6']:0;
			$_SESSION['ans']=substr($_POST['ladate'],0,4);	$_SESSION['mois']=substr($_POST['ladate'],5,2);
			$_SESSION['mois2']=substr($_POST['ladate2'],5,2);$_SESSION['jr_debut']=substr($_POST['ladate'],8,2);$_SESSION['jr_fin']=substr($_POST['ladate2'],2,2);
			...
			if((!empty($_POST['edit6']))||(!empty($_POST['edit12'])))
			  {if(($_SESSION['nuite']<=15)&&($_SESSION['nuite']>=0))
...
/* 			     if(!empty($_POST['edit6']))
					{header('location:reserverch.php?menuParent=Gestion des Réservations');
				     ...
				     if(!empty($_POST['edit6']))
						...
						echo " alert('Aucune r&eacute;servation ne doit pas exc&eacute;der <span style ='font-style:italic;color:red;'> 15 jours</span>. Veuillez contacter l'administrateur');";
						echo "</script>";}
		  ...
		  $de=mysqli_query($con,"UPDATE nombre_reservation SET nbre_salle='".$_SESSION['nbsal1']."'WHERE nbre_chambre='0' OR nbre_salle='0'");
	}else	{ //$msg1="<span style ='font-style:italic;color:red;'>Aucune réservation ne doit être antérieure à la date du jour</span>";
...
?>
<html>
	<head><link rel="Stylesheet" href='css/table.css' />
	...
	<div align="" style="">
		<table align='center' width='800' id="tab"> <tr> <td>
			<form action='reservationsal.php?menuParent=Gestion des Réservations' method='post' name='form1' onSubmit='Verif(); '>
				<table width='' height='400' style='margin-left:70px;'>
					<tr>
						<td colspan="4">
							<h3 style='text-align:center; font-family:Cambria;color:maroon;font-weight:bold;'>RESERVATION DE SALLES </h3>
						</td>
					</tr>
					<tr>
					...
					<tr><input type='hidden' name='edit2' value="<?php echo $Date_actuel2 ?>" readonly />
						...
						<td><input type='text' name='edit1' id='edit1' readonly style='' value="<?php	if(!empty($sal))echo $_SESSION['numresch']; else echo $numReserv;?>"/>
						</td>
						<?php
						  $res=mysqli_query($con,'SELECT count(codesalle) AS nbre FROM salle');
							$ret=mysqli_fetch_assoc($res);$nbre=$ret['nbre'];
							if(!empty($sal))	 
								echo "<td><br/> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Nombre de salles : </td>	
										<td><br/> <input type='number'  min='0' max=".$nbre."  name='edit6' style='width:175px;font-family:sans-serif;font-size:85%;' onkeypress='testChiffres(event);'/>
									</td>";		
						else		{	
						echo "<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Nom du groupe :</td>
						<td>			
						<select name='combo1' style='width:210px;font-size:0.9em;'  />
								<option value=''>  </option>";
					...
					<tr>
						...
						echo "<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Nombre de salles : </td>	";						
						
						echo
						"<td><input type='number' min='1' max=".$nbre."  name='edit6' style='' onkeypress='testChiffres(event);'/>
						</td>
					</tr>";
						}
					?>
