...
$result =   mysqli_query($con, 'SELECT * FROM boisson,config_boisson,conditionnement,QteBoisson WHERE QteBoisson.id=boisson.Qte AND conditionnement.id=boisson.Conditionne AND config_boisson.id=boisson.Categorie AND pc=0 AND  Depot = \'2\' AND (LibCateg LIKE \'%' . safe( $_GET['qt'] ) . '%\'  OR designation LIKE \'%' . safe( $_GET['qt'] ) . '%\' OR LibConditionne LIKE \'%' . safe( $_GET['qt'] ) . '%\'
