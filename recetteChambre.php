...
include 'connexion.php'; 
 echo "	<table align='center' style='margin-left:15%'>
		...
		$ref=mysql_query("SELECT DISTINCT datencaiss FROM encaissement WHERE datencaiss>='".$_SESSION['debut']."' and datencaiss<='".$_SESSION['fin']."' ORDER BY datencaiss ");		
		$somme0=0;$somme1=0;$somme2=0;$somme3=0;$somme4=0;$somme5=0;$somme6=0;$somme7=0;$somme8=0;$somme9=0;$somme10=0;$somme11=0;$somme12=0;
		//echo "SELECT compte.N_reel,compte.np FROM fiche1,compte WHERE fiche1.numfiche=compte.numfiche AND compte.numfiche=encaissement.ref AND fiche1.datarriv >='".$_SESSION['debut']."' AND fiche1.datarriv <='".$_SESSION['fin']."' ORDER BY fiche1.datarriv";
		$var=0;
		...
		{		echo"<tr> "; 		
				...
				echo"<tr bgcolor='' style='font-weight:bold; font-size:16px; '> "; 
					...
					// chambres ventilles simple
						$res=mysql_query("SELECT sum(tarif*np) as som,sum(np) AS np FROM encaissement,chambre WHERE chambre.numch=encaissement.numch AND datencaiss>='".$_SESSION['debut']."' and datencaiss<='".$_SESSION['fin']."' and chambre.typech='V' and encaissement.typeoccup='individuelle' AND Type_encaisse IN ('Entree chambre','Encaissement chambre','Sortie chambre','Impayee fiche individuelle','Facture de groupe')");
						$rers=mysql_fetch_array($res);
						echo"<td>  ";if($rers){echo $somme1=round($rers['som'],4);$somme1=($somme0+$somme1);} else {echo '-';} echo"</td> ";				
						// chambres ventilles double
						$res=mysql_query("SELECT sum(tarif*np) as som,sum(np) AS np FROM encaissement,chambre WHERE chambre.numch=encaissement.numch AND datencaiss>='".$_SESSION['debut']."' and datencaiss<='".$_SESSION['fin']."' and chambre.typech='V' and encaissement.typeoccup='double' AND Type_encaisse IN ('Entree chambre','Encaissement chambre','Sortie chambre','Impayee fiche individuelle','Facture de groupe')");
						$rers=mysql_fetch_array($res);
						echo"<td>  ";if($rers){echo $somme2=round($rers['som'],4);$somme2=($somme0+$somme2);} else {echo '-';}echo"</td> ";					
						// chambres climatise simple
						$res=mysql_query("SELECT sum(tarif*np) as som,sum(np) AS np FROM encaissement,chambre WHERE chambre.numch=encaissement.numch AND datencaiss>='".$_SESSION['debut']."' and datencaiss<='".$_SESSION['fin']."' and chambre.typech='CL' and encaissement.typeoccup='individuelle' AND Type_encaisse IN ('Entree chambre','Encaissement chambre','Sortie chambre','Impayee fiche individuelle','Facture de groupe')");
						$rers=mysql_fetch_array($res);
						...
						// TVA	
						// chambres ventilles simple						
						$res=mysql_query("SELECT sum(tarif*np) as tva FROM encaissement,chambre,exonerer_tva WHERE encaissement.ref =exonerer_tva.numfiche AND chambre.numch=encaissement.numch AND date>='".$_SESSION['debut']."' and datencaiss<='".$_SESSION['fin']."' and chambre.typech='V' and encaissement.typeoccup='individuelle' AND Type_encaisse IN ('Entree chambre','Encaissement chambre','Sortie chambre','Impayee fiche individuelle','Facture de groupe')"); 
						$rers=mysql_fetch_array($res);  $exonerer_tva=round(0.18*$rers['tva'],4); 
						$ref1=mysql_query("SELECT sum(0.18*tarif*np) as tva,sum(np) AS np FROM encaissement,chambre WHERE chambre.numch=encaissement.numch AND datencaiss>='".$_SESSION['debut']."' and datencaiss<='".$_SESSION['fin']."' and chambre.typech='V' and encaissement.typeoccup='individuelle' AND Type_encaisse IN ('Entree chambre','Encaissement chambre','Sortie chambre','Impayee fiche individuelle','Facture de groupe')"); 
						...
						// chambres ventilles double
						$res=mysql_query("SELECT sum(tarif*np) as tva FROM encaissement,chambre,exonerer_tva WHERE encaissement.ref =exonerer_tva.numfiche AND chambre.numch=encaissement.numch AND date>='".$_SESSION['debut']."' and datencaiss<='".$_SESSION['fin']."' and chambre.typech='V' and encaissement.typeoccup='double' AND Type_encaisse IN ('Entree chambre','Encaissement chambre','Sortie chambre','Impayee fiche individuelle','Facture de groupe')"); 
						$rers=mysql_fetch_array($res);  $exonerer_tva=round(0.18*$rers['tva'],4); 
						$ref1=mysql_query("SELECT sum(0.18*tarif*np) as tva,sum(np) AS np FROM encaissement,chambre WHERE chambre.numch=encaissement.numch AND datencaiss>='".$_SESSION['debut']."' and datencaiss<='".$_SESSION['fin']."' and chambre.typech='V' and encaissement.typeoccup='double' AND Type_encaisse IN ('Entree chambre','Encaissement chambre','Sortie chambre','Impayee fiche individuelle','Facture de groupe')"); 
						...
						// chambres climatise simple
						$res=mysql_query("SELECT sum(tarif*np) as tva FROM encaissement,chambre,exonerer_tva WHERE encaissement.ref =exonerer_tva.numfiche AND chambre.numch=encaissement.numch AND date>='".$_SESSION['debut']."' and datencaiss<='".$_SESSION['fin']."' and chambre.typech='CL' and encaissement.typeoccup='individuelle' AND Type_encaisse IN ('Entree chambre','Encaissement chambre','Sortie chambre','Impayee fiche individuelle','Facture de groupe')"); 
						$rers=mysql_fetch_array($res);  $exonerer_tva=round(0.18*$rers['tva'],4); 
						$ref1=mysql_query("SELECT sum(0.18*tarif*np) as tva,sum(np) AS np FROM encaissement,chambre WHERE chambre.numch=encaissement.numch AND datencaiss>='".$_SESSION['debut']."' and datencaiss<='".$_SESSION['fin']."' and chambre.typech='CL' and encaissement.typeoccup='individuelle' AND Type_encaisse IN ('Entree chambre','Encaissement chambre','Sortie chambre','Impayee fiche individuelle','Facture de groupe')"); 
						...
						$res=mysql_query("SELECT * FROM exonerer_tva WHERE date>='".$_SESSION['debut']."' and date<='".$_SESSION['fin']."'"); 
						$rers=mysql_fetch_array($res);  $exonerer_tva=$rers['date']; 
						$res=mysql_query("SELECT * FROM exonerer_aib WHERE date>='".$_SESSION['debut']."' and date<='".$_SESSION['fin']."'"); 
						$rers=mysql_fetch_array($res);  $exonerer_aib=$rers['date']; 
						
				    $var=$somme1+$somme2+$somme3+$somme4+$somme5+$somme6+$somme7+$somme8+$somme9+$somme10+$somme11+$somme12;
						$res=mysql_query("SELECT sum(ttc_fixe*np) AS ttc_fixe FROM encaissement WHERE datencaiss>='".$_SESSION['debut']."' and datencaiss<='".$_SESSION['fin']."'AND Type_encaisse IN ('Entree chambre','Encaissement chambre','Sortie chambre','Impayee fiche individuelle','Facture de groupe')"); 
						while ($ret=mysql_fetch_array($res)) 
							{			//$var=$ret['ttc_fixe']; 
							}
					if(empty($exonerer_tva)&& empty($exonerer_aib))
				...
				echo"</tr> "; 
		echo"</table> ";$aib=0;
	    $ref1=mysql_query("SELECT DISTINCT encaissement.numch AS numch,encaissement.typeoccup AS typeoccup,np AS np FROM exonerer_aib,encaissement,chambre  WHERE chambre.numch=encaissement.numch AND datencaiss>='".$_SESSION['debut']."' and datencaiss<='".$_SESSION['fin']."' AND numfiche=encaissement.ref AND Type_encaisse IN ('Entree chambre','Encaissement chambre','Sortie chambre','Impayee fiche individuelle','Facture de groupe')"); 
