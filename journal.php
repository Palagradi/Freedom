...
<html>
	...
	<body bgcolor='azure'>
		<form  action='' method='post' name='journal'>
			<table align='center'style='font-size:1.1em;'>				
				<tr> 
					<td style='color:#DA4E39;'> <center> <font color='green' size='6' ><br/> JOURNAL DES ENCAISSEMENTS</font> </center> </td> 
				</tr>
			</table>
	 <br/> 
			<center>  <font size='5' >Pour consulter le journal à une date donnée, sélectionnez la période</font> </center>
			<table align='center' >
				<tr>
				...
				<td><br/><input type='submit' name='ok' value='OK' style="border-radius: 5px;-moz-border-radius: 5px;-webkit-border-radius: 5px;font-weight:bold;"/></td> 
	...
	<?php echo "</form>";?>
		...
		{	echo "<table align='center' width='700' border='0' cellspacing='0' style='margin-top:20px;border-collapse: collapse;font-family:Cambria;'>
			...
			if(($_SESSION['poste']!='directrice')&&($_SESSION['poste']!='admin')) 
				echo "Pour rendre compte à une autorité supérieure, <a href='cloture2.php' style='text-decoration:none;color:blue;'>Cliquez ici</a>";
			echo"</span>  </caption> 
	...
	{	$date=date('Y-m-d'); $heure=date('H:i:s');$etat=1; 
	    $sql=mysql_query("update cloture SET encaisseur='".$_POST['edit6']."' WHERE etat=1 ");
		//echo "update cloture SET encaisseur='".$_POST['edit6']."' WHERE etat=1 ";
		$msg="<tr> <td style='font-weight:bold;'> L'encaissement est en attente auprès de la/du <span style='color:red;'> ".$encaisseur."</span>. Veuillez patienter le temps qu'il encaisse.</td></tr>";
	}
	?>
