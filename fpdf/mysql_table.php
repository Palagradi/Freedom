...
function ImproveTable($header1, $data2)
{
	// Largeurs des colonnes
	$w = array(20, 20, 20, 20, 20, 20, 20, 20, 15, 15, 15, 15, 15, 40);
	// En-tête
	...
	{
		$this->Cell($w[$j],6,$row,1,0,'R');
		$j++;
	}
}

function ImproveTable2($header1, $data2, $lignes)
{
	// Largeurs des colonnes
	$w = array(40, 18, 18, 18, 18, 18, 18, 18, 18, 13, 13, 13, 13, 13, 26);
	// En-tête
	for($i=0;$i<count($header1);$i++)
		$this->Cell($w[$i],7,$header1[$i],1,0,'C');
	$this->Ln();
	...
	for($i=0;$i<$lignes; $i++)
	{
		for($j=0; $j<15;$j++)
		{
			if($j==0)
			{
				$this->Cell($w[$j],6,$data2[$i][$j],1,0,'L');
			}
			else
			{
				$this->Cell($w[$j],6,$data2[$i][$j],1,0,'R');
			}
		}
		$this->Ln();
	}
