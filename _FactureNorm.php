...
{ /* 	function temporaire( $texte )
	...
	{  	if($this->PageNo()==1)
		...
		if(isset($nomHotel)) $this->Cell(0,0,$nomHotel,0,1,'');
		//$this->Ln('0');
		...
		$Date_actuel=!empty($date_emission)?$date_emission:$Date_actuel;
		...
		if(isset($logo)) $this->Image($logo,10,15,30,20);
		$this->SetFont('Arial','',10);

		$this->SetTextColor(0,0,0);
			//$x= $this->GetX();$y= $this->GetY();
	// $this->SetXY( $x, $y+75 );
		$this->fact_dev( "Facture N°: ","$initial_fiche"); //$_SESSION['modulo']=0;//$_SESSION['avanceA']=0;
/* 		if(isset($_SESSION['remise'])&& ($_SESSION['remise']>0))
		...
		//else
			...
			// else
				$this->addDate($avanceA); //origine : occup.php
		...
		if($Mtotal<$avanceA)
			{$titre="ARRHES";  if($modulo>0) $difference= $modulo ; //else $difference= abs($difference - $_SESSION['modulo']) ;
		}
		else {
			if(isset($remise)&& ($remise>0))
				{$titre="REMISE"; $Mtotal+=$remise ; $difference=$remise; }
			else
				$titre="SOLDE";
		...
		if(($Mtotal< $avanceA)&&($modulo > 0))  $Mtotal = $avanceA - $modulo;
		...
		$this->SetTextColor(0,0,0);
		$this->addReglement($nom." ".$prenom);

		$this->SetTextColor(0,0,0);
		$this->addEcheance( "Hébergement");
		//$_SESSION['fin']="04-04-2014";
		$this->addNumTVA(  "Du"." ".$debut." "."au"." ".$fin);
		$this->Ln('10');
		}
	...
	$this->Rect(5, 275, 200, 15, "D");
        ...
        //$this->Cell( $this->GetPageWidth(), 7, "Clause de r�serve de propri�t� (loi 80.335 du 12 mai 1980) : Les marchandises vendues demeurent notre propri�t� jusqu'au paiement int�gral de celles-ci.", 0, 0, 'C');
/*
        $y1 = 270;
...
//Positionnement � 1,5 cm du bas
		...
		if(isset($nomHotel)&& isset($footer1))
			$this->Cell(-200,10,' '.$nomHotel.' '.$footer1 ,0,0,'C');
		if(isset($footer2))
			$this->Cell(0,18,$footer2,0,0,'C');
...
function FancyTable($header, $data)
{
    ...
    $this->Cell(array_sum($w),0,'','T'); $x= $this->GetX();$y=$this->GetY();
           ...
           // $this->SetFont('Arial','B',8); $this->SetXY( 181, 227 ); $this->Cell( 24, 6, number_format($_SESSION['TotalHT'], 2, ',', ' '), 0, 0, 'R');
			...
			if(isset($QRCODE_MCF))
			{
			$msg = isset($_GET['msg']) ? $_GET['msg'] : $QRCODE_MCF;
			...
			if(isset($NIM_MCF)) { $this->SetXY( 45, $y+15 ); $this->Cell( 38, 5, "MECeF NIM : ", 0, 0, 'L'); $this->Cell( 38, 5,$NIM_MCF, 0, 0, 'L');
			}
			if(isset($COMPTEUR_MCF)) {
			$this->SetXY( 45, $y+22 ); $this->Cell( 38, 5, "MECeF Compteurs : ", 0, 0, 'L'); $this->Cell( 38, 5,$COMPTEUR_MCF, 0, 0, 'L');
			}
			if(isset($DT_HEURE_MCF)) {
			$this->SetXY( 45, $y+30 ); $this->Cell( 38, 5, "MECeF Heure : ", 0, 0, 'L'); $this->Cell( 38, 5,$DT_HEURE_MCF, 0, 0, 'L');
			}
			if(isset($DT_HEURE_MCF)) {
			$this->SetXY( 45, $y+40 ); $this->Cell( 38, 5, " ISF : ", 0, 0, 'L'); $this->Cell( 38, 5,$ISF, 0, 0, 'L');
			}
	...
	$taux_tva =12;
      ...
      $this->SetFont('Arial','',8);
            ...
            $this->SetXY( 130, $y+10+6 ); $this->Cell( 25, 6, "TVA Collect�e", 0, 0, 'R');
            $this->SetXY( 130, $y+10+6+6 ); $this->Cell( 25, 6, "Taxe de s�jour", 0, 0, 'R');
            $this->SetXY( 130, $y+10+6+6+6 ); $this->Cell( 25, 6, "Total TTC", 0, 0, 'R');
			$this->SetXY( 130, $y+10+6+6+6+6 ); $this->Cell( 25, 6, "Remise accord�e", 0, 0, 'R');
			$this->SetXY( 130, $y+10+6+6+6+6+6 ); $this->Cell( 25, 6, "Net � payer ", 0, 0, 'R');
           ...
           // mysqli_free_result($res);
			...
			if(isset($SIGNATURE_MCF))
			$this->Cell( 65, 8, " CODE MECeF/DGI : ".$SIGNATURE_MCF, 0, 0, 'C');
            // en bas � droite
			...
			$this->SetFont('Arial','B',10); $this->SetXY( 140, $y+2 ); $this->Cell( 47, 8, "D�tails sur la Facturation", 0, 0, 'C');
...
$client=isset($groupe1)?$groupe1:"ee";
...
$NumIFU=!empty($NumIFU)?"\n".$NumIFU:NULL;
if(isset($AdresseClt)&& !empty($AdresseClt)) { $AdresseClt=$AdresseClt; } else if(isset($EmailClt))  $AdresseClt=$EmailClt;else  $AdresseClt=NULL;
$TelClt=!empty($TelClt)?"\n".$TelClt:NULL;
//
...
// Titres des colonnes
$header = array('N�', 'Nom & Pr�noms', 'Chambre', 'Nuit�e', 'Montant HT','TVA','Taxe','Montant TTC');
// Chargement des donn�es
...
//$_SESSION['avanceA']=0;
//$_SESSION['montant']=0;
$defalquer_impaye=0;unset($date_emission);
?>
