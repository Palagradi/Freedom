...
/* 				$customerIFU=$data['NumIFU']; if($customerIFU==0) $customerIFU=0;
				...
				$_SESSION['objet'] =isset($operateur['2'])?$operateur['2']:NULL;
		...
		?>
			<form action="reediterFacturee.php?menuParent=Facturation<?php   if(isset($_GET['avoir'])) echo "&avoir=".$_GET['avoir'];  ?>	" method="POST" style='text-align:center;font-family:cambria;'>
				...
				if(isset($_GET['SIGNATURE_MCF'])&& !isset($_GET['px']))  {
				...
				}).then((value) => { var Es = value;  document.location.href="resetinvoice.php?menuParent=Facturation&SIGNATURE_MCF='.$_GET['SIGNATURE_MCF'].'&px="+Es;
				}); ';
				echo "</script>";
			...
			<div id="resultsC">
				...
				<table align='center' width="80%"  border="0" cellspacing="0" style="border-collapse: collapse;font-family:Cambria;">
					...
					<?php //if(isset($_GET['avoir']))   
						$tx="ETABLISSEMENT D'UNE FACTURE D'AVOIR"; 
					//else  $tx="LISTE DE FACTURES NORMALISEES"; 
					?>
	...
	/* 				if (isset($_POST['ok'])&& $_POST['ok']=='Rechercher')
							...
							// else
								...
								$query_Recordset1 = "SELECT DISTINCT id_request,produitsencours.Client,e_mecef.COMPTEUR_MCF,e_mecef.SIGNATURE_MCF,id_request,DT_HEURE_MCF,clients,operateur,Montant FROM e_mecef,produitsencours WHERE e_mecef.SIGNATURE_MCF=produitsencours.SIGNATURE_MCF
							...
							{	$i++;
								...
								echo " 	<tr bgcolor='".$bgcouleur."'>";
										...
										if(substr($data['COMPTEUR_MCF'],0-2)=="FV")
												echo " &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href='resetinvoice.php?menuParent=Facturation&avoir=1&SIGNATURE_MCF=".$data['SIGNATURE_MCF']."'><img src='logo/b_drop.png' alt='Version imprimable' title='Annuler la facture' width='20' height='20' border='0'></a>";
										else
